...
# --- GRAPHIQUE UNIQUE (Remplacer cette section) ---
fig, ax = plt.subplots(figsize=(10, 9))
ax.set_xlim(-1, 11); ax.set_ylim(-4, 6.5); ax.axis('off')

# 1. ÉTIQUETTES ET POPULATION (Position haute : 4.8 à 6.2)
...
for i in range(20):
    ...
    # Flèche vers le père (xy descend maintenant à 1.5)
    if st.session_state.id_pere == i:
        ax.annotate("", xy=(2.5, 1.5), xytext=(mx, my-0.3), 
    ...
    # Flèche vers la mère (xy descend maintenant à 1.5)
    if st.session_state.id_mere == i:
        ax.annotate("", xy=(7.5, 1.5), xytext=(fx, fy-0.3), 
                    arrowprops=dict(arrowstyle="->", color="gold", lw=2, connectionstyle="arc3,rad=0.2"))

# 2. ZONE DES PARENTS (Position descendue à y = 0.5)
if st.session_state.id_pere is not None:
    dessiner_label(ax, 2.5, 1.5, "Père tiré au hasard") # Étiquette descendue
...
if st.session_state.id_mere is not None:
    dessiner_label(ax, 7.5, 1.5, "Mère tirée au hasard") # Étiquette descendue
...
# 3. ZONE DE L'ENFANT (Position descendue à y = -2.5)
if st.session_state.enfant:
    dessiner_label(ax, 5, -1.2, "Enfant") # Étiquette descendue
    dessiner_indiv(ax, 5, -2.5, st.session_state.enfant)
    
    # Flèches d'allèles (xytext part maintenant de la nouvelle position des parents)
